...
function zipMap2D<U, V, R>(
  width: number,
  height: number,
  values1: U[][],
  values2: V[][],
  f: (value1: U, value2: V) => R
): R[][] {
  let results: R[][] = []
  for (let x = 0; x < width; x++) {
    results[x] = []
...
export default function generateGround(width: number, height: number): PathGround {
  ...
  exits[0][random(0, width - 1)] = true
  exits[height - 1][random(0, width - 1)] = true

  return randomPath(width, height, walkable, exits)
}
